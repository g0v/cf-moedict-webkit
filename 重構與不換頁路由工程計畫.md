...
- **絕對不能**：在未理解既有腳本依賴前任意刪除或改動 inline script，以免破壞原始行為。
...
- **絕對不能**：一次性搬移所有頁面並同步刪除舊結構；必須逐檔遷移，隨時驗證行為。

## 階段 2：路由狀態與事件集中管理
- [x] 設計內部路由狀態（History API vs. Hash routing），評估與既有 hash 片段（`#=*`、`#@`）兼容方式。
- [x] 在 `MainLayout` 中實作路由解析器：將 hash/path 拆解為 `{ view: 'dictionary' | 'about' | 'radical' | 'starred', lang, payload }`，提供給子元件使用。
- [x] 將現有散落在 `generateHTMLWrapper` 內的 click/hash 攔截邏輯整理成集中式 handler，透過 Preact 狀態更新與 `history.pushState`/`replaceState` 控制。
- [x] 定義「類似 Vue emit」的機制：透過 props 傳入 callback 或 Context，讓 `Navbar`、`DictionaryView`、`RadicalView` 等觸發路由切換時僅改變父層狀態。
- **絕對不能**：直接在新版路由中呼叫 `window.location` 或 `window.history` 以整頁刷新；必須保持 SPA 導航流程。

## 階段 3：資料載入與重繪策略
- [ ] 字典資料的取得方式：直接在瀏覽器端呼叫現有 `/:text.json` API
...
- [ ] 確認路由變換時，只有主要部份的DOM更換，Navbar不刷新。
- **絕對不能**：繞過 API 函式直接 `fetch` HTML 或 Worker，否則無法重用同一套處理流程。
- **絕對不能**：一次改寫所有資料流程，應逐個 view 漸進抽換並保持可回退。
...
- **絕對不能**：在未完成資料層重構前就啟動 hydration，以免前後端狀態不一致導致閃動。
